...
async def _handle_chat(websocket: WebSocket, data: dict):
    ...
    if use_skills:
        base_sys += (
            ...
            "  - application/vnd.pimio.benchmark+json — body: JSON {title?, "
            "subtitle?, device?, runs:[{label, prefill_tps, decode_tps, "
            "ttft_ms?, memory_gb?, acceptance?}]}. Use for matched MLX "
            "performance comparisons; never invent measurements.\n"
            "  - application/vnd.pimio.model-card+json — body: JSON {name, "
            "family?, parameters?, quantization?, format?, context_window?, "
            "size_gb?, memory_gb?, drafters?, features?, revision?, source?, "
            "sha256?}. Use for model/checkpoint compatibility summaries.\n"
            "  - application/vnd.pimio.inference-trace+json — body: JSON {title?, "
