...
from mio.dflash.kernels import tape_replay_kernel
...
class RecurrentRollbackCache(_BaseCache):
    ...
    def filter(self, batch_indices):
        ...
        # Keep rollback artifacts in sync with the filtered batch so that
        # downstream rollback(n_accepted) replays the correct row's tape.
        if self._snapshot is not None:
            self._snapshot = [
                s[batch_indices] if s is not None else None for s in self._snapshot
            ]
        for attr in ("_tape", "_tape_k", "_tape_g", "_tape_qkv"):
