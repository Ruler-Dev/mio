...
from mio.turboquant.rotation import generate_rotation_matrix, generate_jl_matrix, build_combined_rot_jl
from mio.turboquant.kernels import (
    turboquant_encode,
    qjl_encode,
    pack_2bit_indices,
    unpack_2bit_indices,
...
def _pack_indices(indices: mx.array, bits: int) -> mx.array:
    if bits == 2:
        return pack_2bit_indices(indices)
    if bits == 3:
        return pack_3bit_indices(indices)
    if bits == 4:
        return pack_4bit_indices(indices)
    raise ValueError(f"Unsupported mse_bits={bits}; supported: 2, 3, 4")


def _unpack_indices(packed: mx.array, D: int, bits: int) -> mx.array:
    if bits == 2:
        return unpack_2bit_indices(packed, D)
    if bits == 3:
        return unpack_3bit_indices(packed, D)
    if bits == 4:
        return unpack_4bit_indices(packed, D)
    raise ValueError(f"Unsupported mse_bits={bits}; supported: 2, 3, 4")
...
class TurboQuantKVCache:
    ...
    def get_key_indices(self) -> mx.array:
        ...
        return _unpack_indices(self.key_packed, self.head_dim, self.mse_bits)

    def get_value_indices(self) -> mx.array:
        """Unpacks value indices. Returns (..., D) uint32."""
        return _unpack_indices(self.value_packed, self.head_dim, self.mse_bits)
    ...
    def state(self, v):
        if v is None or len(v) == 0:
            ...
            self.offset = 0
            return
        expected = 6 if self.use_qjl else 4
        if len(v) != expected:
            raise ValueError(
                f"TurboQuantKVCache expected {expected} state arrays, got {len(v)}"
            )
        self.key_packed = v[0]
        self.key_norms = v[1]
        ...
        if self.use_qjl:
            ...
            self.key_sign_bits = None
            self.key_residual_norms = None
        self.offset = int(self.key_packed.shape[2])
    ...
    def meta_state(self, v):
        if v is None or v == "" or len(v) == 0:
            return
        offset = int(v[0])
        if self.key_packed is not None and offset > int(self.key_packed.shape[2]):
            raise ValueError("TurboQuantKVCache offset exceeds restored state")
        self.offset = offset
