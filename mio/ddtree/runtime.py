...
def generate_ddtree_once(
    *,
    ...
    Args:
        ...
        quantize_kv_cache: If True, use mlx_lm QuantizedKVCache (8-bit group 64)
            on full-attention layers. Requires DDTREE_EXACT_COMMIT=1 (set by
            caller) so commit runs sequentially — tree_aware_path_commit can't
            write per-position into quantized (values, scales, biases) tuples.
    ...
    # Create caches. When quantize_kv_cache=True, full-attention layers use
    # mlx_lm QuantizedKVCache (8-bit, group 64). DDTree's tree_aware_path_commit
    # can't slice per-position into quantized (values, scales, biases) tuples,
    # so the caller must also set DDTREE_EXACT_COMMIT=1 to route commits through
    # the sequential-forward path instead.
    ...
    def _run_dflash_cycle(block_len: int) -> tuple[int, bool, float]:
        ...
        if token_callback is not None and emitted_ids:
            token_callback(list(emitted_ids))
    ...
    while len(generated_tokens) < max_new_tokens:
        ...
        if token_callback is not None and emitted_token_ids:
            token_callback(list(emitted_token_ids))
        start += n_accepted
        target_hidden = committed_hidden
...
def stream_ddtree_generate(
    *,
    ...
    """Generator wrapper for DDTree producing DFlash-shaped event dicts.

    Runs generate_ddtree_once on a background thread and yields events to the
    caller as tokens are committed:
        {"event": "token", "token_id": int}  — one per accepted token
        {"event": "summary", ...full generate_ddtree_once result}

    Mirrors the shape of mio.dflash.runtime.stream_dflash_generate so it drops
    into MioEngine.generate_stream without shape adapters.
    """
    ...
    def _on_tokens(token_ids: list[int]) -> None:
    ...
    def _worker() -> None:
        try:
            result_holder["summary"] = generate_ddtree_once(
                target_model=target_model,
            ...
            q.put(_SENTINEL)
        except BaseException as exc:  # re-raise on main thread
    ...
    while True:
        ...
        kind, tid = item
        yield {"event": kind, "token_id": tid}
    thread.join()
    summary = dict(result_holder.get("summary") or {})
    summary["event"] = "summary"
    summary.setdefault("prompt_token_count", len(prompt_tokens))
    yield summary
