...
# Use mio.dflash's model navigation helpers (handles VL models, nested wrappers).
# Routed SDPA dispatches to quantized_sdpa when the cache is a QuantizedKVCache,
# so tree verify works alongside 8-bit KV compression.
...
from mlx_lm.models.base import scaled_dot_product_attention as _routed_sdpa
    ...
    *,
    ...
    if tree_mask_only:
        ...
        if should_split:
            ...
            # Route through mlx_lm's SDPA wrapper so a QuantizedKVCache on
            # `cache` dispatches to mx.fast.quantized_scaled_dot_product_attention.
            output = _routed_sdpa(
