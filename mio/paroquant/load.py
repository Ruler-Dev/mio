...
def load(model_path: str, lazy: bool = False, force_text: bool = False) -> tuple:
    ...
    # Pre-cache and eagerly evaluate rotation parameters for all PARO layers.
    # The custom Metal rotation kernel produces NaN when cos/sin are lazy
    # arrays in the computation graph (buffer aliasing in lazy eval).
    # Evaluating them once at load time fixes this with zero inference overhead.
    _warm_rotation_caches(model)
...
def _warm_rotation_caches(model) -> None:
    """Pre-compute and evaluate rotation parameters for all PARO layers."""
    arrays_to_eval = []
    for _name, mod in model.named_modules():
        if isinstance(mod, RotateQuantizedLinear):
            if not mod._cached:
                mod._cache_rotation()
            arrays_to_eval.extend([mod._cos, mod._sin, mod._scales_flat])
        elif isinstance(mod, RotateSwitchGLU):
            if not mod._cached:
                mod._cache_rotation()
            for tag in ["_gate_up_rot", "_down_rot"]:
                arrays_to_eval.extend([
                    getattr(mod, f"_rot{tag}_cos"),
                    getattr(mod, f"_rot{tag}_sin"),
                    getattr(mod, f"_rot{tag}_scales_flat"),
                ])
    if arrays_to_eval:
