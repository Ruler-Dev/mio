...
promote the current controller or reopen an observed split as held-out data.
...
DFlash timing. V3 added raw active-phase timing and native receipts, then its
only smoke aborted before hidden evaluation because terminal wall time could
exceed a budget without an exhaustion attestation. V4 binds those receipts and
repairs that accounting with one final same-sample wall probe. It does not
...
| Paired scoring ablation | Native stride 1 and FP32-renormalized stride 8 produced identical text, token IDs, token counts, and finish reasons on 32/32 tasks. The native/FP32 scoring-time ratio was 4.978x in the joint FP32/stride-8 condition, while total adapter generation was 0.153% slower. | Paired AUROC and AURC intervals crossed zero and point estimates favored native scoring. There is no evidence of ranking-signal improvement and no end-to-end speedup. |

The held-out split remains sealed. The next admissible work is a 2x2
calibration-only ablation that separates precision from scoring stride,
followed by coarser hierarchical pooling or an external calibration corpus. A
held-out run requires a new, frozen preregistration and must occur only after
those gates pass.
